...
test_that("covariate Stein-Fojo effects can be centred independently", {
    ...
    for (name in c("b", "s", "g")) {
        ...
        expect_true(all(
            initialValues(model, n_chains = 1)[[1]][[paste0(
                "lm_sfc_psi_",
                name
            )]] >
                0
...
test_that("covariate Stein-Fojo reduces to the original formulation", {
    ...
    expected <- list(
        mu_b = old_parameters$prior_const_lm_sf_mu_bsld[
            old_indexes$subject_study_index
        ],
        omega_b = old_parameters$prior_const_lm_sf_omega_bsld[
            old_indexes$subject_study_index
        ],
        mu_s = old_parameters$prior_const_lm_sf_mu_ks[
            old_indexes$subject_arm_index
        ],
        omega_s = old_parameters$prior_const_lm_sf_omega_ks[
            old_indexes$subject_arm_index
        ],
        mu_g = old_parameters$prior_const_lm_sf_mu_kg[
            old_indexes$subject_arm_index
        ],
        omega_g = old_parameters$prior_const_lm_sf_omega_kg[
            old_indexes$subject_arm_index
        ]
    )
...
test_that("SimLongitudinalSteinFojoCov uses covariate predictors", {
    ...
    expect_equal(
        sampled$psi_b,
        c(rlnorm(1, log(60), 0.2), rlnorm(1, log(120), 0.3))
    )
...
test_that("Stein-Fojo covariate model recovers its parameters", {
    ...
    predictor_truth <- c(
        ...
        mu_s_intercept = log(0.5),
        mu_s_coefficients = log(0.8),
        omega_s_intercept = log(0.2),
        omega_s_coefficients = log(1.25),
        mu_g_intercept = log(0.2),
        mu_g_coefficients = log(1.3),
        omega_g_intercept = log(0.15),
    ...
    simulated <- SimJointData(
        ...
        longitudinal = do.call(
            ...
            c(
                list(
                    times = seq(-0.25, 2.5, length.out = 16),
                    mu_b_formula = ~arm,
                    omega_b_formula = ~arm,
                    mu_s_formula = ~arm,
                    omega_s_formula = ~arm,
