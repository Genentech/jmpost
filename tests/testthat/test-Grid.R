...
test_that("GridObservered + Constructs correct quantities", {
    ...
    longquant_manual <- LongitudinalQuantities(
        ...
        grid = GridManual(
            spec = list(
                "pt_0004" = dat_lm |> dplyr::filter(pt == "pt_0004") |> dplyr::arrange(time) |> dplyr::pull(time),
                "pt_0002" = dat_lm |> dplyr::filter(pt == "pt_0002") |> dplyr::arrange(time) |> dplyr::pull(time),
                "pt_0050" = dat_lm |> dplyr::filter(pt == "pt_0050") |> dplyr::arrange(time) |> dplyr::pull(time)
            )
        )
    )
...
test_that("GridPopulation() works as expected", {
    ...
    samples_df <- as.CmdStanMCMC(mp)$draws(
