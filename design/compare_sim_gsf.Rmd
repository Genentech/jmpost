...
not_needed <- mp_k@results$draws()
saveRDS(mp_k, file = "mp_k.rds")
readRDS("mp_k.rds")
...
## Now make a nice plot for the hex logo

```{r}
few_pat <- head(s1_data_os$pt, 20)
long_samples <- longitudinal(mp_k, patients = few_pat)
autoplot(long_samples)
surv_samples <- survival(mp_k, patients = few_pat)
autoplot(surv_samples, add_km = FALSE)
```

Let's do it for patient 6 (longitudinal trend going up, survival of course going down).

```{r}
large_size <- 100
make_ggplot_of_funs <- function() {
    object <- long_samples[6]
    ...
    long_color <- "#69b3a2"
    surv_color <- rgb(0.2, 0.6, 0.9, 1)
    
    library(ggplot2)
    p <- ggplot() +
        geom_line(aes(x = .data$time, y = .data$median), data = all_fit_df, color = long_color) +
        geom_ribbon(aes(x = .data$time, ymin = .data$lower, ymax = .data$upper), data = all_fit_df, alpha = 0.3, color = long_color, fill = long_color) +
        geom_point(aes(x = .data$t, y = .data$y), data = all_obs_df, color = long_color) +
        xlab(expression(t)) +
        scale_y_continuous(
            # Features of the first axis
            name = "y(t)",
            # Add a second axis and specify its features
            sec.axis = sec_axis( trans=~./600, name="S(t)")
        ) +
        theme(
            axis.title.y = element_text(color = long_color, size = large_size),
            axis.title.y.right = element_text(color = surv_color, size = large_size)
        )
    
    object <- surv_samples[6]
    ...
    p <- p +
        geom_line(aes(x = .data$time, y = .data$median * 600), data = all_fit_df, color = surv_color) +
        geom_ribbon(aes(x = .data$time, ymin = .data$lower * 600, ymax = .data$upper * 600), data = all_fit_df, alpha = 0.3, color = surv_color, fill = surv_color)
    
    p
}

make_hexplot <- function(out_path = "./man/figures/hex8.png") {
  # Dependancies
  require(hexSticker)
  require(showtext)
  ...
  hexSticker::sticker(
    ...
    h_color = "#094F26",
    h_size = 2,
    url = "github.com/Genentech/jmpost",
    u_size = 24,
    filename = out_path,
    p_family = "mono",
