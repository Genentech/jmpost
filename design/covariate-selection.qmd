...
On a high level, we have two groups of methods: (i) those that perform covariate selection implicitly through the use of shrinkage priors, and (ii) those that perform covariate selection explicitly by defining a model space and then selecting the best model in this space. The first group includes horseshoe and R2-D2 priors, while the second group includes projection predictive variable selection and posterior probabilities for covariate selection.
...
4. (Only if initial path is deemd overoptimistic) Perform a more extensive search of the covariate selection model space, using cross-validation to guide the search.
