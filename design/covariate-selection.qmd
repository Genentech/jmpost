...
- `slab` has an inverse gamma prior with `df_slab/2` parameters. Note that after multiplying with `scale_slab^2` and obtaining `c2`, this corresponds to $c^2 \sim \text{Inv-Gamma}(\alpha, \beta)$ with $\alpha = \text{df\_slab}/2$ and $\beta = \text{df\_slab} * \text{scale\_slab}^2 / 2$ in the above equations
...
This is the same regularized horseshoe construction in spirit, with a few minor differences. First, `brms` uses a non-centred parameterization: the coefficient is represented as a standard normal variable $z_j$ multiplied by the regularized scale $\tau\tilde{\lambda}_j$. Marginally, this gives the same as above,
