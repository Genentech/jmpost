...
#' @param lambda_cen Lambda for censoring distribution (1/mean in years)
...
#' @param pt_suffix A string to attach to the end of the pt name in order to ensure its unique
#' across multiple runs of the function (for different arms / studies)
#'
#' @import dplyr
...
simulate_os_sld <- function(
    ...
    time_gap <- 1 / 365
    psi_names <- c("b", "s", "g", "phi")

    assert_that(
        all(psi_names %in% names(psi_omega)),
        all(psi_names %in% names(psi_mu)),
        msg = paste(
            "Either psi_mu or psi_omega don't contain at least",
            "one of 'b', 's', 'g' or 'phi'"
        )
    )
    ...
    psi_df <- tibble(
        var = psi_names,
        mu = unlist(psi_mu[psi_names]),
        omega = unlist(psi_omega[psi_names])
    )


    sld_pars_meta <- eta %>%
        gather(var, eta, -pt) %>%
        left_join(psi_df, by = "var")
    ...
    dat <- expand.grid(pt = unique_pts, time = time_vals, stringsAsFactors = FALSE) %>%
        ...
        left_join(blcovariates, by = "pt") %>%
        mutate(dslg = dSLD(time_offset, b, s, g, phi)) %>%
        mutate(log_bl_haz = log_haz_loglogistic(time_offset, lambda, p)) %>%
        mutate(log_haz_dsld = beta * dslg) %>%
        mutate(log_haz_ttg = gamma * ttg) %>%
        mutate(hazard_per_interval = time_gap * exp(
            log_bl_haz + log_haz_cov + log_haz_dsld + log_haz_ttg
        )) %>%
        group_by(pt) %>%
        mutate(chazard = cumsum(hazard_per_interval)) %>%
        filter(chazard <= chazard_limit) %>%
        mutate(time = time * 365) %>%
        group_by(pt) %>%
        mutate(final = row_number() == n()) %>%
        ungroup() %>%
        select(pt, time, SLD, age, sex, final, b) %>%
        arrange(pt, time)
