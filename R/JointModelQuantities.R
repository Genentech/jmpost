...
setOldClass("CmdStanGQ")
#' `JointModelQuantities`
#'
#' Contains samples and generated quantities from a [`JointModel`].
#'
#' @slot model (`StanModule`)\cr the original model.
#' @slot data (`list`)\cr data input.
#' @slot init (`list`)\cr initial values.
#' @slot results (`CmdStanMCMC`)\cr the results from [sampleStanModel()].
#' @slot quantites (`CmdStanGQ`)\cr the results from [generateQuantities()].
#'
#' @aliases JointModelQuantities
#' @export
.JointModelQuantities <- setClass(
    "JointModelQuantities",
    slots = c(
        model = "StanModule",
        data = "list",
        init = "list",
        fit = "CmdStanMCMC",
        quantities = "CmdStanGQ"
    )
)


#' @rdname JointModelQuantities-class
#'
#' @param joint_model (`JointModelSamples`)\cr the Model Samples Object.
#' @param generated_quantities (`CmdStanGQ`)\cr A `CmdStanGQ` object.
#'
#' @export
JointModelQuantities <- function(joint_model, generated_quantities) {
    .JointModelQuantities(
        model = joint_model@model@stan,
        data = joint_model@data,
        init = joint_model@init,
        fit = joint_model@results,
...
#' `validate_time_grid`
#'
#' This function performs basic assertions to ensure that a user-provided `time_grid`
#' meets the basic assumptions (ordered, finite, unique, not-NULL).
#' @param time_grid (`numeric`)\cr vector of time points
#' @keywords internal
validate_time_grid <- function(time_grid) {
    assert_that(
        ...
        msg = "`time_grid` needs to be finite, sorted, unique values numeric vector"
    )
}


# longitudinal-JointModelQuantities ----
...
setMethod(
    f = "longitudinal",
    signature = c(object = "JointModelQuantities"),
    definition = function(object, patients = NULL, ...) {
        ...
        y_fit_at_grid_samples <- object@quantities$draws("y_fit_at_time_grid")
        y_fit_samples <- object@fit$draws("Ypred")[, 1L, , drop = TRUE]
        results <- list()
...
setMethod(
    ...
    definition = function(object, patients = NULL, ...) {
        ...
        time_grid_index <- seq_along(time_grid)
        log_surv_at_grid_samples <- object@quantities$draws("log_surv_fit_at_time_grid")
        log_surv_at_obs_samples <- object@fit$draws("log_surv_fit_at_obs_times")[, 1L, , drop = TRUE]
        results <- list()
        for (this_pt in patients) {
            this_result <- list()
