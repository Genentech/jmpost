...
setMethod("data_prep",
    ...
    function(
        ...
        object@data_sld[, object@vars$long_user_id] <- factor(
            object@data_sld[, object@vars$long_user_id, drop = TRUE]
        )

        object@data_os[, object@vars$treatment] <- factor(
            object@data_os[, object@vars$treatment, drop = TRUE]
        )

        # Derive sparse matrices.
        obs_y_dat <- object@data_sld[
            object@data_sld[, object@vars$longitudinal] >= cens_threshold,
        ]
        mat_inds_obs_y <- t(
            model.matrix(
                as.formula(
                    paste("~ -1 +", object@vars$long_user_id)
                ),
                data = obs_y_dat
            )
        )
        ...
        cens_y_dat <- object@data_sld[
            object@data_sld[, object@vars$longitudinal] < cens_threshold,
        ]
        mat_inds_cens_y <- t(
            model.matrix(as.formula(paste("~ -1 +", object@vars$long_user_id)),
                data = cens_y_dat
            )
        )
        ...
        for (i in 1:length(treat_arms)) {
            n_index_per_arm[i] <- sum(
                object@data_os[, object@vars$os_arm, drop = TRUE] == treat_arms[i]
            )
        }
        ...
        arm_study_trt <- unique(
            object@data_os[, c(object@vars$os_arm, object@vars$os_study_id, object@vars$treatment)]
        )

        arm_to_study_values <- as.integer(
            factor(
                arm_study_trt[, object@vars$os_study_id, drop = TRUE]
            )
        )
        ...
        .jm_data(
            ...
            data = list(
                ...
                study_index = as.numeric(
                    factor(
                        object@data_os[, object@vars$os_study_id, drop = TRUE],
                        levels = studies_name
                    )
                ),
