...
- $h_0(.)$ is a parametric baseline hazard distribution
- $t$ is the event time
- $\theta$ is a vector of parameters that parameterise the baseline hazard distribution
- $\psi_i$ is am arbitrary vector of parameters for subject $i$ specified by the Longitudinal model
...
baseline hazard distribution $h_0(.)$. Please note that some of these distributions do note have
the proportional-hazards property meaning that the resulting $h_i(.)$ distribution will not be of 
the same parameteric family has $h_0(.)$.

## Log-Logistic Distribution

$$
h(x \mid a, b) =  \frac
{(b/a)\left(x/a\right)^{(b-1)}}
{1 + (x/a)^b}
$$
