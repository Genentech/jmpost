...
In order to avoid unnecesary processing, code that is only used for generated quantities is
only included into the Stan model when explicitly trying to extract the quantities via the
`LongitidunalQuantities` and `SurvivalQuantities` constructors.

To facilitate this, when using `LongitidunalQuantities` or `SurvivalQuantities` 
a dedicated model method `enableGQ()` is called on the longitudinal model. This model specific
...
implmentation of this method for the Random-Slope model which implements both feature (2) and (3)
