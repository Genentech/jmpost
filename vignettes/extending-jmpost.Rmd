...
Survival distributions are implemented as an S4 objects that inherit from `SurvivalModel`.
...
1) In the `parameters` block each of the distributions parameters must be formall declared.


2) In the `transformed parameters` block there must define a vector called `pars_os` which contains 1
...
- The `time` matrix contains 1 row per unique subject and 1 column per timepoint for that subject
