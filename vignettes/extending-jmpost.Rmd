...
Each longitudinal model is responsible for defining their own implmentations of the $\phi$ vector.
...
For reference the following is roughly the implmentation for the `LongitudinalGSF` model:
...
into the final Stan model. For reference the following is roughly the implmentation of the
...
`linkIdentity()` and `linkTTG()` respectively. For example the following is the actual implmentation
