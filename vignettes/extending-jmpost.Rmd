...
1) In the `transformed parameters` block there must define a vector called `pars_os` which contains one
element for each parameter used by the survival distribution.


2) In the `functions` block there must define a function called `log_h0` which returns the
