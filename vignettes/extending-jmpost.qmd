...
The `parameters` slot is a `ParameterList()` object which is used to declare each parameter
used by the survival distribution and specify its prior distribution, as well as to declare
the design matrix coefficients
(please see the "Prior Specification" section below).

The `stan` slot is a `StanModule()` object which needs to define the following:

1) In the `transformed parameters` block you must define a vector called `pars_os` which contains one
element for each parameter used by the survival distribution.


2) In the `functions` block you must define a function called `log_h0` which returns the
...
- The `pars_os` vector is as defined from (1) above and contains the parameters for that
...
the `stan` and `parameters` slots which specify the underlying Stan code and the model parameter
declarations and prior distributions respectively (please see the "Prior Specification" section below).
...
For a complete worked implementation, including simulation, fitting, link functions, and generated
quantities, see [Fitting a Custom Longitudinal Model](custom-model.html).
...
## Custom Longitudinal Models with Covariates

Custom longitudinal models can make population parameters depend on subject-level (baseline) covariates.
The covariates belong in the data supplied to `DataSubject()`; the `DataLongitudinal()` formula
remains an outcome-and-time formula such as `sld ~ time`. The custom model is responsible for
turning those subject variables into any design matrices declared by its Stan module.

In addition to the interfaces described above, a covariate-aware model usually implements the
following methods:

- `longitudinal_model_stan_data.<class>(model, subject)` returns the model-specific design-matrix
  dimensions and values used when fitting. The method should build its rows in the harmonised
  `DataSubject` order.
- `required_longitudinal_covs.<class>(object, ...)` returns the subject variables required by the
  population-mean predictors. `GridPopulation()` uses this to validate its `newdata` profiles.
- `gq_population_stan_data.<class>(object, model, data, ...)` returns a list with `declarations`
  and `data`. The declarations are inserted into the generated-quantities Stan data block, while
  `data` contains the corresponding design matrices evaluated for `object@newdata`.
- `enableGQ.<class>()` constructs population parameters from those generated-quantities design
  matrices. The design matrix for `newdata` must preserve the fitted factor levels, contrasts,
  and column order.

For example, a model containing the Stan declarations
...
can supply them at fitting time with an S3 method of the form

```R
longitudinal_model_stan_data.MyModel <- function(model, subject) {
    design <- model.matrix(
        model@baseline_formula,
        data = as.data.frame(subject)
    )[, -1, drop = FALSE]
    list(
        p_baseline_cov = ncol(design),
        baseline_cov_design = design
    )
}
```

For predictions, call `GridPopulation(newdata = ...)` with one complete subject profile per row.
At minimum, each profile contains the study, arm, and every variable named by
`required_longitudinal_covs()`. When factors are present, do not build the prediction matrix from
`newdata` alone: use the fitted subject data as the reference so that its levels and contrasts are
reused.

See [Fitting a Custom Longitudinal Model with Covariates](custom-cov-model.html) for a complete and runnable example. It defines separate covariate predictors, injects their Stan data, fits a joint
model, and implements both individual and covariate-aware population generated quantities.

## Prior Specification

When writing your own custom longitudinal or survival model it is important to understand
how model parameters and prior definitions are specified. By default `jmpost` will insert
the Stan declarations and prior statements for entries in the `parameters` slot of the model
object. This means that parameters listed in `ParameterList()` should not be declared in the
custom Stan code itself, and their prior distributions should not be defined there either.
Note that hierarchical relationships between parameters are still model-specific and must be
specified in the Stan code. For example in the `LongitudinalRandomSlope` model there is a
different random slope for each treatment arm, and this relationship is specified in the Stan
code as:
