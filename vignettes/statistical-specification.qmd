...
The horseshoe prior is a global-local shrinkage prior [@CarvalhoEtAl2010]. The global parameter shrinks all coefficients towards zero, while coefficient-specific local parameters allow some coefficients to escape the shrinkage. The regularized horseshoe prior [@PiironenVehtari2017] additionally introduces a slab component that regularizes coefficients that are far from zero. This is useful for covariate selection because all candidate covariates can be included in one model fit, with irrelevant coefficients strongly shrunk towards zero.

For $p$ covariate coefficients $\beta_{j}, j = 1, \dotsc, p$, the regularized horseshoe prior applied with `prior_horseshoe()`, and with positive hyperparameters $\nu_{\lambda}, \nu_{\tau}, \nu_c, s_{\tau}, s_c$, can be written as
...
Equivalently, conditional on $\lambda_j$, $\tau$, and $c$, this implies
...
A useful summary of the amount of shrinkage applied to coefficient $j$ is the shrinkage factor
...
Values of $\kappa_j$ close to 1 indicate strong shrinkage towards zero, while values close to 0 indicate little shrinkage. Regularized horseshoe priors have been applied in PK/PD covariate selection by @PourzanjaniDavis2026, where the main practical advantage is the ability to assess all candidate covariates in a single Bayesian model fit.



# Longitudinal Model Specification

The longitudinal models in `jmpost` have each two versions: one without covariates, and one with covariates.
In each of the below subsections, we describe both versions. The models without covariates are simpler, and comprise parameters which are study- and treatment-arm-specific and correspondingly indexed.
To start with, we describe the commonalities of the longitudinal models having covariates.

## Models with Covariates {#models-with-covariates}

In the longitudinal models with covariates, the study- and treatment-arm-specific parameters of the longitudinal models without covariates are replaced by subject-specific predictors. For a model parameter $\theta$, let
$\boldsymbol{x}_{\theta i}$ be the baseline covariates for subject $i$ and let
$\boldsymbol{\beta}_{\theta}$ be their coefficients, while $\boldsymbol{\alpha}_{\theta}$ is the intercept. The model parameter value for subject $i$ may then be derived from the intercept and slope parameters together with the covariates by one of the following forms:

$$
\theta_{i} =
\begin{cases}
\alpha_{\theta} + \boldsymbol{x}_{\theta i}^{\top}\boldsymbol{\beta}_{\theta}
    & \text{(linear)}, \\
\alpha_{\theta}\left(1 + \boldsymbol{x}_{\theta i}^{\top}\boldsymbol{\beta}_{\theta}\right)
    & \text{(proportional)}, \\
\alpha_{\theta}^{\,\boldsymbol{x}_{\theta i}^{\top}\boldsymbol{\beta}_{\theta}}
    & \text{(exponential)}, \\
\exp\!\left(\alpha_{\theta} +
    \boldsymbol{x}_{\theta i}^{\top}\boldsymbol{\beta}_{\theta}\right)
    & \text{(log-linear)}.
\end{cases}
$$

Standard-deviation parameters can only use the exponential or log-linear form, because these are strictly positive.
The exponential form is
$\alpha_{\theta}^{\,\boldsymbol{x}_{\theta i}^{\top}\boldsymbol{\beta}_{\theta}}$
with $\alpha_{\theta}>0$, and the log-linear form is
$\exp(\alpha_{\theta}+\boldsymbol{x}_{\theta i}^{\top}\boldsymbol{\beta}_{\theta})$.
The exponential form fixes the reference-profile standard deviation at one.
When selecting it, the intercept prior must support a positive base; the
default scale-intercept priors are chosen for the log-linear form.

A couple of additional points worth calling out:

- Different covariates and parameterisations may be used for each $\theta$.
- Note that we use here location-scale models, where the mean and standard deviation of the response variable are modeled separately.
- Regarding the detailed implementation in the Stan code, we have introduced the options to switch between centered and non-centered parameterizations for the random effects. By default the baseline random effects use the centered parameterization, because it has been found to generally improve the sampling efficiency and convergence of the model for the baseline parameters. On the other hand, the random effects of all other parameters by default use the non-centered parameterization. See the arguments of the constructors for the respective longitudinal models with covariates for details, for example `?LongitudinalSteinFojoCov` describes the arguments `centred_baseline`, `centred_shrinkage` and `centred_growth`.
...
- $s_i$ is the random slope for subject $i$ with $s_i \sim N(\mu_{sk(i)}, \sigma_{sk(i)})$
- $\mu_{sk(i)}$ is the mean for the random slope within treatment arm $k(i)$
- $\sigma_{sk(i)}$ is the variance term for the slope in treatment arm $k(i)$
...
Following [models with covariates](#models-with-covariates), the random-slope model then becomes

$$
\begin{align*}
y_{ij} &= \mu_i + s_i t_{ij} + \epsilon_{ij}, \\
s_i &\sim N\!\left(\mu_{s i},\ \sigma_{s i}\right).
\end{align*}
$$

Any predictor form above can be used for $\mu_i$. The mean $\mu_{s i}$ and standard deviation $\sigma_{s i}$ of the random slope standard deviation $s_i$ use either the exponential or log-linear parametrization. For example, under the log-linear parametrization,
$\log(\sigma_{s i}) = \alpha_{\sigma_s} + \boldsymbol{x}_{\sigma_s i}^{\top}\boldsymbol{\beta}_{\sigma_s}$.

The original model is recovered by using study indicators for the covariates of $\mu_i$ and treatment-arm indicators for the the covariates of $\mu_{s i}$ and $\sigma_{s i}$.
...
Following [models with covariates](#models-with-covariates),
here we let $\theta_{i}$ be a subject-specific predictor for parameter $\theta$, constructed from parameter-specific baseline covariates. The standard-deviation predictors $\omega_{\theta i}$ can only use the exponential or log-linear form.

That means all population parameters, $\mu_{b i}$, $\omega_{b i}$, $\mu_{s i}$, $\omega_{s i}$, $\mu_{g i}$, and $\omega_{g i}$, are constructed from subject-specific predictors.

The original
model is recovered by using study indicators for the predictors of $b_i$ and $\omega_{b i}$ and
treatment-arm indicators for those of $s_i$ and $\omega_{s i}$ as well as those of $g_i$ and $\omega_{g i}$.
...
When generating population quantities for the $b$, $s$, and $g$
parameters, the median of the distribution is used. This has the same
interpretation as using a non-centred parameterisation and setting the random
effect to zero.
...
The logit-normal distribution is parameterised here as
$\operatorname{logit}(\phi_i) \sim
N(\mu_{\phi k(i)}, \omega_{\phi k(i)})$.
...
* $k(i)$ is the treatment arm index for subject $i$
* $l(i)$ is the study index for subject $i$
* $\mu_{\theta k(i)}$ is the population location on the transformed scale for parameter $\theta$ in group $k(i)$
* $\omega_{\theta k(i)}$ is the population standard deviation on the transformed scale for parameter $\theta$ in group $k(i)$.
...
s_i &= \exp(\mu_{sk(i)} + \omega_{s k(i)} * \eta_{s i}) \\
g_i &= \exp(\mu_{gk(i)} + \omega_{g k(i)} * \eta_{g i}) \\
\phi_i &= \text{logit}^{-1}(\mu_{\phi k(i)} + \omega_{\phi k(i)} * \eta_{\phi i}) \\
\\
\eta_{b i} &\sim  N(0, 1)\\
\eta_{s i} &\sim   N(0, 1)  \\
\eta_{g i} &\sim   N(0, 1)  \\
...
That means all population parameters, $\mu_{b i}$, $\omega_{b i}$, $\mu_{s i}$, $\omega_{s i}$, $\mu_{g i}$, $\omega_{g i}$, $\mu_{\phi i}$, and $\omega_{\phi i}$, are constructed from subject-specific predictors.

The original model is recovered by using study indicators for the predictors of $b_i$ and $\omega_{b i}$ and treatment-arm indicators for those of $s_i$ and $\omega_{s i}$, $g_i$ and $\omega_{g i}$, and $\phi_i$ and $\omega_{\phi i}$.
...
That means all population parameters, $\mu_{b i}$, $\omega_{b i}$, $\mu_{g i}$, $\omega_{g i}$, $\mu_{c i}$, $\omega_{c i}$, $\mu_{p i}$, and $\omega_{p i}$, are constructed from subject-specific predictors.

The original model is recovered by using study indicators for the predictors of $b_i$ and $\omega_{b i}$ and treatment-arm indicators for those of $g_i$ and $\omega_{g i}$, $c_i$ and $\omega_{c i}$, and $p_i$ and $\omega_{p i}$.
...
When generating population quantities for the $b$, $g$, $c$ and $p$ parameters the median
of the distribution is used. This is because this has the same interpretation as using a 
non-centred parameterisation and setting the "random effects" term to be 0.
