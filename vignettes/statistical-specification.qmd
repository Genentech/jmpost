...
\mu_i &= \eta_{\mu i}, \\
s_i &\sim N\!\left(\eta_{\mu_s i},\ \exp(\eta_{\sigma_s i})\right).
\end{align*}
$$

Here we can use a linear, proportional, or exponential parameterisation for $\eta_{\mu i}$,
and typically use a log-linear parametrization for $\eta_{\sigma_s i}$, which means that $\log(\sigma_{s i}) = \eta_{\sigma_s i} = \alpha_{\sigma_s} + \boldsymbol{x}_{\sigma_s i}^{\top}\boldsymbol{\beta}_{\sigma_s}$.
...
constructed from parameter-specific baseline covariates as in the linear,
proportional, or exponential forms above. The population parameters become
...
When generating population quantities for the $b$, $s$, $g$, and $\phi$
...
effect to zero. In particular, the population quantity for $\phi$ is
$\operatorname{logit}^{-1}(\mu_\phi)$.
