...
In general terms, we can specify the vector of relevant parameters as $\theta = (μ(x), \alpha(x))$. In this notation, consistent with **flexsurv**, we consider: 
...
knitr::kable(ztab, 
    escape = F, 
    caption = 'Table 1. Location and shape parameters for the distributions implemented in jmpost', 
...
mp<-sampleStanModel(jm, data=jdat, iter_warmup=4000, 
...
### 3.2 Descriptive outputs

The most efficient way of extracting relevant outputs from the MCMC samples is to use the **posterior** package.
In the below snippet, after having reformatted the model results with `posterior::as_draws_df(mp@results)`, all  samples across chains are available in `my_fitall_df`.

The **posterior** package can then be used to extract the relevant quantities.
...
Traceplots (a.k.a. caterpillar plots), Rhat, effective sample size (ESS) and autocorrelation in the samples have to be examined for convergence to be considered as acceptable. 

To this extent a [recommendation](https://cran.r-project.org/web/packages/bayesplot/vignettes/visual-mcmc-diagnostics.html#effective-sample-size) of the Stan development team is to study these measures of convergence using the **bayesplot** package.

Other interesting quantities, such as posterior quantiles or highest density interval (HDI) for any given parameter can also be extracted using the **tidybayes** package. 

In the following code, we extract various measures of uncertainty associated with the mean or the median of the exponential rate (`sm_exp_lambda`).
...
Finally, the posterior distributions for any given model parameter (across chains or overall) can be easily displayed graphically using the **bayesplot** package.
...
In this section, we illustrate how to overlay the fitted survival curves for each patient respectively and the Kaplan-Meier curve for the studied sample.

We start by extracting the individual location parameter estimate.
