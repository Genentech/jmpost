...
Is the clinical benefit for the patients good enough to warrant a large Phase 2 or Phase 3 study?
Or should we rather combine this drug with another drug to achieve better clinical benefit? 
The challenge is that in Phase 1 studies we don’t follow the patients until death but only until the cancer is growing again (progression event). 
Therefore we don’t have reliable overall survival data. 
...
Specifically, we can correlate the tumor response data (i.e. the shrinkage and growth of tumor lesions over time) with the overall survival of the patients @beyer2020multistate. 
...
In late drug development for cancer, one of the most frequently used measurement types is time to death or overall survival. 
It has been suggested that overall survival can be correlated with longitudinal measurements of different types according to the type of cancer.
In such cases, longitudinal measurements can be used for the prediction of the overall survival probability or hazard ratios over time.
Time projectories of survival probabilities are particularly important for decision making during the drug development process.

The sum of the length diameter of the cancer (SLD) provides an effective and representative biomarker for longitudinal measurements in solid cancer. 
The SLD is relatively easy to measure with great accuracy while also being a good representative of the development of cancer. 

The association between the longitudinal data and the overall survival can be achieved in different ways.
The predicted SLD value or its first derivative over time can be directly treated as the linking factor.
...
Compared to other R-packages, `jmpost` provides the flexibility of non-linear modeling for the longitudinal part of the joint model. 
...
```{r}
my_long_mod <- LongModel(stan = StanModule(
