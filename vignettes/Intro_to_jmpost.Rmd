...
subtitle: "`J`oint `M`odel for `P`osterior `O`verall `S`urvival `T`ranjectories"
package: jmpost
author:
...
The sum of the longest diameter of the cancer (SLD) provides an effective and representative biomarker for longitudinal measurements in solid cancer. 
SLD is relatively easy to measure with great accuracy while also being a good representative of the development of cancer. 
...
Compared to other R-packages, `jmpost` provides the flexibility of non-linear mixed effects modeling for the longitudinal part of the joint model. 
