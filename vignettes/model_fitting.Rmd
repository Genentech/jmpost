...
column names to the required variables happens. Note that we need to load the
`survival` package because we use `Surv()` in the overall survival model for the 
covariate variables specification.
...
This is using the `cmdstanr` functions, because the result object is of class
`CmdStanMCMC`.
