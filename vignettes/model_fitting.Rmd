...
warmup and sampling iterations, here we are just trying to keep the vignette
...
using the corresponding method, and overlay the curves with the Kaplan-Meier plots:

```{r, warning=FALSE}
groups <- split(os_data$pt, os_data$arm)
surv_grouped_samples <- aggregate(survival(mcmc_results), groups = groups)
autoplot(surv_grouped_samples)
```
