...
And using the `extractSurvivalQuantities()` method we can do the same for the estimated survival
functions.

```{r fig.width=6}
surv_quantities <- extractSurvivalQuantities(
    mcmc_results,
    patients = selected_patients
)
as.data.frame(surv_quantities)
summary(surv_quantities)
