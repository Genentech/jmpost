...
subjects with any arbitrary covariate and TGI model parameter values. This is done via passing a
`GridPrediction()` object to the method where the `newdata` and `params` arguments are used to
specify the desired values for the covariates and TGI model parameters respectively.
...
The exact names that are required for the `params` argument (i.e. the TGI model parameters) can be
